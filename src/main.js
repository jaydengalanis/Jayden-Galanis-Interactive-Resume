...
// Frutiger Aero sky — that iconic 2007 sky blue gradient
const skyColor = new THREE.Color(0x87ceeb);
const horizonColor = new THREE.Color(0xc8e8f5);
...
// Clouds — simple billboard-style puffs
...
// ─────────────────────────────────────────
// LIGHTS — warm Frutiger Aero sunlight
// ─────────────────────────────────────────
...
// Screen glow — warm amber vintage monitor glow visible from afar
const screenGlow = new THREE.PointLight(0xffcc44, 2.5, 6);
screenGlow.position.set(0, 1.9, 0.2);
scene.add(screenGlow);

// Subtle fill around desk area
const deskFill = new THREE.PointLight(0xffeedd, 0.8, 4);
deskFill.position.set(0, 2.5, 1.5);
scene.add(deskFill);

// ─────────────────────────────────────────
// SCREEN GLOW MESH (visible before zoom-in)
// A glowing plane that sits on the monitor face
// ─────────────────────────────────────────
...
const glowPlane = new THREE.Mesh(new THREE.PlaneGeometry(1.595, 1.49), glowPlaneMat);
// Default position — will be overridden by ScreenPlane if found
  ...
  (gltf) => {
    ...
    landscape.traverse(child => {
      if (child.isMesh) {
        ...
        // Override the lake material to be transparent + animated
        if (child.name === 'Lake') {
          ...
          lakeRef = child; // save reference to animate it
        }
      }
    });
    scene.add(landscape);
    ...
    landscape.traverse(child => {
      if (child.isMesh && child.name === 'Terrain') {
        terrainMesh = child;
      }
    });

    if (terrainMesh) {
      createGrass(terrainMesh);
    }
  }
);
...
loader.load(
  ...
  (gltf) => {
    ...
    if (screenRef) {
      ...
      // Also position the far-glow plane at the screen
...
// --- screen-inner should be replaced with glass aero in the future.
...
cssObject.position.set(0, 1.9, -0.25);
cssObject.scale.setScalar(0.00135);
...
muteBtn.addEventListener('click', () => {
  ...
  // ── Toggle muted class for visual state
...
function animate() {
  ...
  // Screen glow pulse — warm amber flicker like a real CRT
